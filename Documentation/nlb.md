# AWS Network Load Balancer (NLB) Logs Fetcher Usage Guide

## Basic Command

```bash
python aws-logs-generic.py --access-key YOUR_AWS_ACCESS_KEY --secret-key YOUR_AWS_SECRET_KEY --role-arn arn:aws:iam::YOUR_ACCOUNT_ID:role/YOUR_ROLE_NAME --bucket YOUR_S3_BUCKET --service nlb --account-id YOUR_ACCOUNT_ID
```
...
- `--bucket`: The S3 bucket name where NLB logs are stored
- `--service`: Set to 'nlb'
...
- Logs are saved in JSON format with raw content (NLB logs are not JSON)
...
python aws-logs-generic.py \
    ...
    --prefix logs/nlb \
    --region us-east-1 \
    --output nlb_logs.json \
    --once \
    --lookback 120 \
    --log-file nlb_fetch.log
```
